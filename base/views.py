...
class StudentModelView(APIView):
    ...
    def get(self, request, id=-1):
        if id > -1:
            try:
                ...
                return Response('Cannot get a student that does not exist !')
    ...
    def put(self, request, id):  # we couldve also used the update() (see below req.method == 'POST'), same shit... update() is same is instansiating StudentSerializer & save()                  
            ...
            if new_stu_serialized.is_valid():
                ...
                return Response(new_stu_serialized.data, status=status.HTTP_200_OK)
            return Response(new_stu_serialized.errors, status=status.HTTP_400_BAD_REQUEST)
